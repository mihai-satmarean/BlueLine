...
TArray<FBlueLineGraphAnalyzer::FNodeCluster> FBlueLineGraphAnalyzer::DetectNodeClusters(UEdGraph* Graph)
{
    ...
    for (UEdGraphNode* StartNode : Graph->Nodes)
    {
        ...
        if (Cluster.Nodes.Num() > 0)
        {
            ...
            Cluster.Layer = DetermineNodeLayer(StartNode); // Approximation
